...
| 7 | **Prediction Test Commands:**<br/>`curl -X POST "http://localhost:8000/predict" -H "Content-Type: application/json" -d "{\"features\": {\"feature1\": 50.0, \"feature2\": 75.0}}"`<br/>...and batch version | FastAPI test | Sends real input payloads to `/predict` and `/predict/batch` endpoints to return predictions and the model version ID. |
| 8 | `pytest ci_cd/tests/ -v` | CI/CD testing | Runs end-to-end automated tests covering pipeline integrity, data validation, and API predictions. |
| 9 | `git add . && git commit -m "Pipeline updates" && git push origin main` | Version control | Pushes all updated artifacts, tests, and results to your GitHub repo for collaboration and CI/CD automation. |
